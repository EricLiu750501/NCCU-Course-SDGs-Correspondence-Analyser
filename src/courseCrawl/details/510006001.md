...
Acquire knowledge of the usage of complex phrases and mixedsentences -for example,  
...
詞彙練習:單字填空Vocabulary: Fill in the blanks with proper words **thẩm mỹ, bất cập, đánh giá, chăm sóc, hệ thống, sở hữu.** |  2 |  3  
...
**15** |  **Giao Thông** **交通** **Transportation** | 
  ...
  * 文法筆記 Grammatical notes:
  ...
  1. 結構: **Nếu A thì B** 「如果。。。就」的語法。

**Nếu A thì B** (if A then B).
  1. **Vấn nạn** 與**vấn đề** 之差異

Notes on usage: **vấn nạn and vấn đề**.
  1. **Dẫn đến/ đưa đến** : 類似於中文「導致」的用法。

Usage of words **dẫn đến/ đưa đến** “to lead to”.
  1. 運用字彙：**Làm cho / khiến cho** 「使得」。

Usage of words:**Làm cho / khiến cho**.
  1. 「**Một cách** 與複合形容詞搭配」的用法。

**Một cách** + double – adjective. | 
  * 口說練習: 兩人一組，按照句型，回答問題。
...
Speaking Practice: work in pairs, answer to questions based on patterns
  * 回家作業Homework:
  ...
  1. 按照單字來造句 Create the sentences with Vietnamese words: **vấn đề, giải quyết, cải thiện, kẹt xe, đề xuất, làm cho, khiến cho**.
  2. 寫作練習:依據語法，練習。Writing: Drills
...
學生需於課前將詞彙記熟，進行文法與閱讀複習，以便於課堂上進行 討論與練習。課堂作業繳交將以寄送 Moodle方式交予講師。最終成績將依以下五個標準計算:
Students are responsible for memorizing vocabulary, reviewing grammar, and reading materials before class in order to reserve valuable in-class time for oral practice. All assignments are submitted in Moodle prior to the deadlines. The final grades are determined by 5 parts as follows:
1. 出席率與參與度(討論，課堂活動，截止日期) Attendance （10%）and participation （10%） (discussion, in-class activities, meeting deadlines): 20%
2. 每週作業 Weekly homework: 30%
3. 期中考試 Midterm exam: 25%
5. 期末考試 Final Exam: 25%
##  指定/參考書目Textbook & References
本課程「中級越語」由本系東南亞語文學系的老師編著
