...
def extract_scores_from_json(json_data):
    """Extract scores from the specific JSON format where each key is a discipline with a 'score' field."""
    scores = {}
    for discipline, data in json_data.items():
        if isinstance(data, dict) and 'score' in data:
            scores[discipline] = data['score']
    return scores

def compare_json_files(file_paths):
    """Load and compare multiple JSON files."""
    results = []
    for file_path in file_paths:
        ...
        if data:
            ...
            results.append({"Mode": file_name, "Score": list(scores.values()), "Categories": list(scores.keys())})
    return results
...
def plot_score_distributions(mode_scores, title="Score Distributions by Mode"):
    ...
    # Save the plot
    plt.savefig(f"./score_distributions.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def analyze_scores():
    ...
    # Generate paths to the JSON files
    file_paths = [f"./tempResult{i}.json" for i in [1, 4, 5, 6]]
    
    # Compare the JSON files
    comparison_data = compare_json_files(file_paths)
    
    if comparison_data:
        # Extract scores for each file
        mode_scores = {item["Mode"]: item["Score"] for item in comparison_data}
        categories = comparison_data[0]["Categories"] if comparison_data else []
        
        # Plot score distributions
        plot_score_distributions(mode_scores, "Score Comparison Across Files")
        
        # Calculate correlations
        # current_correlations = calculate_correlations(mode_scores)
