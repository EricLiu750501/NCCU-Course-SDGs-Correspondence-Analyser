...
def plot_top_k_scores(llm_scores, k=5, title_prefix="Top Concepts for"):
    """
    Create individual plots showing the top K highest scoring concepts for each LLM.
    
    Args:
        llm_scores: Dictionary with LLM names as keys and dictionaries of concept scores as values
        k: Number of top concepts to display for each LLM (default: 5)
        title_prefix: Prefix for the plot title
    """
    # Process each LLM separately
    for llm_name, scores in llm_scores.items():
        # Sort concepts by score and get top K
        top_concepts = sorted(scores.items(), key=lambda x: x[1], reverse=True)[:k]
        
        if not top_concepts:
            print(f"No concepts found for {llm_name}")
            continue
            
        # Extract names and scores for plotting
        concept_names = [concept for concept, _ in top_concepts]
        concept_scores = [score for _, score in top_concepts]
        
        # Create horizontal bar chart
        plt.figure(figsize=(12, 7))
        
        # Create horizontal bars
        bars = plt.barh(range(len(concept_names)), concept_scores, align='center', 
                        color=plt.cm.viridis(np.linspace(0.2, 0.8, len(concept_names))),
                        alpha=0.8)
        
        # Set y-axis ticks and labels
        plt.yticks(range(len(concept_names)), concept_names, fontsize=12)
        
        # Add score values at the end of each bar
        for i, (bar, score) in enumerate(zip(bars, concept_scores)):
            plt.text(bar.get_width() + 0.1, bar.get_y() + bar.get_height()/2, 
                    f'{score:.2f}', va='center', fontsize=10)
        
        # Set plot labels and title
        plt.xlabel('Score', fontsize=14)
        plt.title(f"{title_prefix} {llm_name}", fontsize=16)
        plt.xlim(0, max(concept_scores) * 1.1)  # Add some space for the text
        plt.grid(axis='x', linestyle='--', alpha=0.6)
        
        # Add a line for the average of top K scores
        avg_score = sum(concept_scores) / len(concept_scores)
        plt.axvline(x=avg_score, color='red', linestyle='--', alpha=0.7)
        plt.text(avg_score + 0.1, len(concept_names) - 0.5, 
                f'Avg: {avg_score:.2f}', va='center', fontsize=10, color='red')
        
        plt.tight_layout()
        plt.savefig(f"{current_folder}/top_{k}_{llm_name}.png", dpi=300, bbox_inches='tight')
        plt.close()
...
def analyze_scores():
    ...
    if llm_scores:
        ...
        plot_top_k_scores(llm_scores, k=3, title_prefix="Top 3 Highest Scoring Concepts")
